...
```text
# Conversion DRAFT: cisco-asa -> srx
...
# CAVEAT: source dst_zone unresolved by parser (metadata warning) — fixture has from-zone=to-zone=outside, but destination WEB (10.0.1.10/32) falls inside INSIDE-NET (10.0.0.0/16, zone "inside"); confirm correct to-zone before commit (see manual items).
set security policies global policy 010-OUTSIDE-IN-1 match from-zone outside
set security policies global policy 010-OUTSIDE-IN-1 match to-zone outside
...
Section: security_policies   → converted-with-caveats (1/1 — action/log map cleanly; destination zone unresolved by parser)
...
Manual items (must do on target):
  1. Resolve security-policy OUTSIDE-IN-1 destination zone. The parsed fixture carries
     from-zone=to-zone=outside with a metadata warning ("ACL destination zone may require
     route/interface resolution beyond access-group binding"). The destination WEB
     (10.0.1.10/32) sits within INSIDE-NET (10.0.0.0/16) on the inside zone, so the policy
     almost certainly needs `match to-zone inside`. Confirm against the real topology and
     correct before commit.
  2. Add explicit inside->outside permit policy if outbound traffic must flow. SRX has no
...
- **`nameif` + `security-level` → SRX zones.** ASA zones are interface labels with a
  ...
  cleanly; the *behavior* the security-levels implied does not, hence manual items 1–2.
